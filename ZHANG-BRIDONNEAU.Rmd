...
\begin{split}
\left[I_1(\beta)^{-1} \right]^{1/2} \sqrt{n}(\widehat{\beta} - \beta) \rightarrow \mathcal{N}(0, I_{dp})
\end{split}
\end{equation}


$I_1(\beta)^{-1}$ est la matrice de l'information de Fisher, noté que $\widehat V = I_1(\beta)^{-1}$ et $I_{dp}$ est la matrice identité de la taille $p$.
...
\begin{split}
\sqrt{n}(\widehat{\beta} - \beta) \rightarrow \mathcal{N}(0, I_1(\beta)^{-1}) \\
\widehat{\beta} \sim \mathcal{N}(\beta, I_1(\beta)^{-1})
\end{split}
\end{equation}
...
$H(\beta)$ est bien une matrice définie négative. Etant donnée la distribution asymptotique normale de l'emv, on peut concluire que $\widehat{Var(\widehat\beta)} = - [H(\widehat{\beta})]^{-1}$, maintenant on calcul $\widehat{Var(\widehat \lambda)}$:
...
La condition de convergence indiquée dans la section 1 est-elle vérifiée? (je ne sais pas)

### Estimer regression linéaire simple
...
#residus_stud <- residus/(sigma_est * sqrt(1-H_x))
#points(X%*%theta_est,residus,main="différents résidus")
#abline(h=2,lty=2)
#abline(h=-2,lty=2)
```
...
Comme on a montré dans le premier section, l'intervalle de confiance pour $\lambda$ est $[\widehat{\lambda} - q_{1-\alpha/2} \sqrt{\dfrac{\widehat{Var(\widehat{\lambda})}}{n}}, \widehat{\lambda} + q_{1-\alpha/2} \sqrt{\dfrac{\widehat{Var(\widehat{\lambda})}}{n}}]$ au niveau asymptotiquement $1-\alpha$. Ici on fixe $\alpha$ à $0.05$, donc $q_{1-\alpha/2}$ est $1.959964$ la quantile d'ordre $1-\alpha/2$ sous loi normale, donc on obtien l'IC $[0.3341304, 0.4293202]$.

```{r message=FALSE, include=FALSE}
alpha <- 0.05
q <- qnorm(1-alpha/2)
lambda_est - q*sqrt(var_lambda_est/n) #lower bound
lambda_est + q*sqrt(var_lambda_est/n) #upper bound
```
...
On définit le modèle linéaire multiple ($M1$) $y = \mu + \beta_1 x_1 + \beta_2 x_2 + \beta_3 x_3 + \sigma\varepsilon$, $\varepsilon \sim \mathcal{N}(0, I_n)$ i.i.d.
...
X = as.matrix(cbind(1,df[-1])) # retirer la première colonne
Y = as.matrix(df$y) #observation de Y

#estimateur de theta
theta.est = solve(t(X)%*%X)%*% t(X)%*%Y # t(X) transposée de X
n = length(Y)
p = ncol(X)
#estimateur sans biais de sigma
sigma.est = sqrt( sum( (X%*%theta.est -Y)^2 ) / (n-p) ) 

#estimation de variance sur l'estimateur de theta
V = solve(t(X)%*%X) *sigma.est^2
# ecart type sur l'estimateur de theta
stddev = sqrt(diag(V))
...
X2 <- as.matrix(cbind(1, longueur, amplitude, chargement))

#estimateur de theta
theta.est2 = solve(t(X2)%*%X2)%*% t(X2)%*%Y # t(X) transposée de X
n = length(Y)
p = ncol(X2)
#estimateur sans biais de sigma
sigma.est2 = sqrt( sum( (X2%*%theta.est2 -Y)^2 ) / (n-p) ) 

#estimation de variance sur l'estimateur de theta
V2 = solve(t(X2)%*%X2) *sigma.est2^2
# ecart type sur l'estimateur de theta
stddev2 = sqrt(diag(V2))

{plot(Y, X2%*%theta.est2, xlab = "Valeurs observées",ylab = "Valeurs ajustées")
abline(0, 1)}
...
```{r echo=FALSE}
res2 <- lm(formula = Y ~ df$x1 + df$x2 + df$x3 + I(df$x1^2) + I(df$x2^2) + I(df$x3^2) + I(df$x1*df$x2) + I(df$x1*df$x3) + I(df$x2*df$x3))

{plot(res2$fitted,df$y,main="ajustés/observés")
abline(0,1)}
```
