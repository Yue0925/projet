...
Supposons que pour $\beta = (\lambda, \theta, \sigma^2)^{'}$ a $p \times 1$ vecteur de paramètres, on ait $h_\lambda(Y_i) = Z_i = x_i \theta + \varepsilon_i, \; \varepsilon_i \sim \mathcal{N}(0,\sigma^2)$, $\varepsilon_i$ suivent une loi gaussien i.i.d. Donc par la définition de vraisemblance:
...
A $\lambda$ fixé, on peut déterminer l'emv par estimation du maximum de vraisemblance. Donc tout d'abord, depuis l'équation \ref{eq1} on calcule le log-vraisemblance.
...
En suite, étant donnée que le log-vraisemblance $\ell$ l'équation \ref{eq2} est une transforamtion monotone de vralsemblance $L$ dans l'équation \ref{eq1}, on maximise log-vraisemblance $\ell$ en respectant $\theta$, $\sigma^2$ et $\lambda$, donc on obtient le premier ordre dérivation ci-dessous:
...
Donc on a $\widehat{\sigma }^2 = \dfrac{(h_{\lambda}(Y)-X\theta)^{'} (h_{\lambda}(Y)-X\theta)}{n} = \dfrac{h_{\lambda}(Y)^{'} (I_n-H) h_{\lambda}(Y)}{n}$, avec $H = X(X^{'}X)^{-1}X^{'}$ et $I_n$ matrice identidé.
...
Pour vérifier $L_{max}(\lambda)$, on remplace notre l'emv $\widehat{\sigma}^2$ et $\widehat{\theta}$ calculé par l'équation \ref{eq3} et \ref{eq4} dans log-vraisemblance $\ell$:
...
Donc $a(n) = -\dfrac{n}{2} -\dfrac{n}{2}\log(2\pi)$ est bien une constante ne dépendant que $n$. Maintenant on calcule l'emv $\widehat{\lambda}$:
...
\pdv{L_{max}}{\lambda} & = -\dfrac{n}{2} \dfrac{1}{\widehat{\sigma}^2(\lambda)} \left|\pdv{\widehat{\sigma}^2(\lambda)}{\lambda} \right| + \sum_{i=1}^n \log\left|Y_i \right| \\
 ...
 & = -\dfrac{(h_{\lambda}(Y)-X\theta)\left|\pdv{h_{\lambda}(Y)}{\lambda} \right|}{\sigma^2}+ \sum_{i=1}^n \log\left|Y_i \right| = 0
\end{split}
\end{equation}

On peut bien vérifier que $\frac{\partial \ell}{\partial \lambda}$ et $\frac{\partial L_{max}}{\partial \lambda}$ sont équaux par calculation l'équation maximum vraisemblance, donc on propose de trouver $\widehat{\lambda}$ qui maximize $L_{max}(\lambda)$ de l'équation \ref{eq5}, c'est à dire minimize $\widehat{\sigma}^2(\lambda)$. Par l'équation \ref{eq3}, on sait que $\widehat{\sigma}^2(\lambda) = \dfrac{||h_{\lambda}(Y)-X\theta||^2}{n} = \dfrac{SCR}{n}$ est la somme de carrés résiduels divisée par $n$, donc on cherche l'emc (estimateur des moindres carrées)

\begin{equation} \label{eq8}
\begin{split}
\widehat{\lambda} & = \argmin_{\lambda} \norm{h_{\lambda}(Y)-X\widehat{\theta}}^2
\end{split}
\end{equation}
...
\begin{split}
\left[\dfrac{V_n(\beta)^{-1}}{n} \right]^{1/2} \sqrt{n}(\widehat{\beta} - \beta) \rightarrow \mathcal{N}(0, I_n)
\end{split}
\end{equation}


$V_n(\beta)^{-1}$ est l'information de Fisher.
...
set.seed(999)
X_obs <- rnorm(n=50)
Epsilon <- rnorm(n=50, mean=0, sd = sqrt(sig_squared))
