...
Lmin = function(lambda, Y=Y) {
  -Lmle(Z) + (1-lambda) * vec1%*%log(abs(Y)) + n/2 + (n/2)*log(2*pi)
}
# Tracer −Lmax
...
```{r echo=FALSE}
resopt = nlm(Lmin,Y=Y,p=0,hessian=TRUE)
resopt
-1/resopt$hessian
```

En utilisant la fonction $nlm$, on obtient la valeur maximum de $L_{max}$ est $-resopt\$minimum$, $\widehat\lambda$ est $resopt\$estimate$. Comme démontré dans l'équation (\ref{eq15}), la variance de $\widehat\lambda$ est l'inverse de hessian fois $-1$.
...
#lam_chap
#var_chap
#lam_chap - q*sqrt(var_chap/n) #lower bound
#lam_chap + q*sqrt(var_chap/n) #upper bound
```
